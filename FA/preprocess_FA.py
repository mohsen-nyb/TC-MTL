...
def load_dataset_classification(path_data, path_label, random_state=42, test_ratio=0.20):
    ...
    labels_df = pd.read_csv(path_label)
    data_idx_list = [4, 238, 259, 633]



    patients0 = labels_df[labels_df['score_calss']==0]['VISITNUM'].unique()
    patients1 = labels_df[labels_df['score_calss'] == 1]['VISITNUM'].unique()
    patients2 = labels_df[labels_df['score_calss'] == 2]['VISITNUM'].unique()
    patients3 = labels_df[labels_df['score_calss'] == 3]['VISITNUM'].unique()
    ...
    for visit_pat in patient_visit_train:
        output_seqs.append(labels_df[labels_df['VISITNUM'] == visit_pat]['score_calss'].values[0])

        p_name = labels_df[labels_df['VISITNUM'] == visit_pat]['PTID'].values[0]
        max_visit_num = labels_df[labels_df['VISITNUM'] == visit_pat]['num'].values[0]
        pt_lst = []
        visitnames = labels_df[labels_df['PTID']==p_name]['VISITNUM'].values
    ...
    for visit_pat in patient_visit_test:
        output_seqs_test.append(labels_df[labels_df['VISITNUM'] == visit_pat]['score_calss'].values[0])
    ...
    return data_input_df, labels_df, patient_visit_train, input_seqs, output_seqs, patient_visit_test, input_seqs_test, output_seqs_test, data_idx_list, class_weights